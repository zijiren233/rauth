...
const (
	...
	DefaultSecretName = "registry-credentials"
)
...
func (c *Client) GetNamespaceCredentials(ctx context.Context, namespace string) (*RegistryCredentials, error) {
	secret, err := c.clientset.CoreV1().Secrets(namespace).Get(ctx, c.secretName, metav1.GetOptions{})
	if err != nil {
		return nil, fmt.Errorf("failed to get secret %s in namespace %s: %w", c.secretName, namespace, err)
	}
...
func ExtractCredentials(secret *corev1.Secret) (*RegistryCredentials, error) {
	...
	if !ok {
		return nil, fmt.Errorf("secret does not contain 'username' key")
	}

	password, ok := secret.Data["password"]
	if !ok {
		return nil, fmt.Errorf("secret does not contain 'password' key")
	}
